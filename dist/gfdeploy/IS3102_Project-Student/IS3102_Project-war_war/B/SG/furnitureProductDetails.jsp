    ...
    <body>
        <%
            ...
            for(int i = 0; i < furnitures.size(); i++){
                if(sku.equals(furnitures.get(i).getSKU())){
                    index = i;
                    break;
                }
            }
                    ...
                    <div class="container">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div>
                                    <div class="thumbnail">
                                        <img alt="" class="img-responsive img-rounded" src="../../..<%=furnitures.get(index).getImageUrl()%>">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="summary entry-summary">
                                    <h2 class="shorter"><strong><%=furnitures.get(index).getCategory()%></strong></h2>
                                    <%
                                        if (isMemberLoggedIn == true) {
                                    %>
                                    <form action="../../ECommerce_AddFurnitureToListServlet">
                                        <input type="hidden" name="id" value="<%=furnitures.get(index).getId()%>"/>
                                        <input type="hidden" name="SKU" value="<%=furnitures.get(index).getSKU()%>"/>
                                        <input type="hidden" name="price" value="<%=furnitures.get(index).getPrice()%>"/>
                                        <input type="hidden" name="name" value="<%=furnitures.get(index).getName()%>"/>
                                        <input type="hidden" name="imageURL" value="<%=furnitures.get(index).getImageUrl()%>"/>
                                        <input type="submit" name="btnEdit" class="btn btn-primary" id="<%=furnitures.get(index).getId()%>" value="Add To Cart"/>
                                    </form>
                                    <%}%>
                                    <p class="price"><h4 class="amount"><%="$" + String.format("%.2f", furnitures.get(index).getPrice())%></h4></p>
                                    <strong>Description</strong>
                                    <p class="taller">
                                        <%=furnitures.get(index).getDescription()%>
                                    </p>
                                    <p>
                                        Height: <%=furnitures.get(index).getHeight()%><br/>
                                        Length: <%=furnitures.get(index).getLength()%><br/>
                                        Width: <%=furnitures.get(index).getWidth()%>
                                    </p>
                                    <div class="product_meta">
                                        <span class="posted_in">Category: <a rel="tag" href="../../ECommerce_FurnitureCategoryServlet?cat=<%=(furnitures.get(index).getCategory().replace("&", "%26")).replace(" ", "+")%>"><%=furnitures.get(index).getCategory()%></a></span>
                                    </div>
                                    <br/><br/>
