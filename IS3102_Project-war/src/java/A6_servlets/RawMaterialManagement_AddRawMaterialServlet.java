...
public class RawMaterialManagement_AddRawMaterialServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            if(SKU.matches("RM[0-9]+")){
                if(_length > 0 && width > 0 && height > 0){
                    if (!itemManagementBean.checkSKUExists("RM" + SKU.replace("RM", "").replaceFirst("^0+(?!$)", ""))) {
                    ...
                    result = "?errMsg=Failed to add raw material, Length, Width, Height must be greater than 1.";
                    response.sendRedirect(source + result);
                }
            }else{
                result = "?errMsg=Failed to add raw material, SKU must be in format: RM?. '?' represents integer > 0.";
                response.sendRedirect(source + result);
            }
